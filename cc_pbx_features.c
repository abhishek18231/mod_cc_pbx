...
switch_status_t handle_call(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path, char *opsip_ip_port, call_details_t *call)
{
        ...
        const char *participant_id = switch_channel_get_variable(channel, "participant_id");
        const char *play_file = switch_channel_get_variable(channel, "play_file");
        const char *application = switch_channel_get_variable(channel, "sip_h_X-application");
        char id[20], did_call[2], gw_limit[2] = {'\0'};
        char *data, *temp_path, *query;
        char **tokl = NULL;
        int num;
        ...
        if (!IS_NULL(caller))
        {
                ...
                if (get_ext_details(channel, &call->caller, dsn, mutex, caller))
                {
                        ...
                        //switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "clrname:%s", call->caller.cid_name);
                        switch_channel_set_variable(channel, "clr_name", call->caller.cid_name);
        ...
        if (!IS_NULL(application) && !strcmp(application, "click2call") && get_ext_details(channel, &call->callee, dsn, mutex, callee))
        {

                switch_channel_set_variable(channel, "application", "intercom");
                switch_channel_set_variable(channel, "cust_id", call->callee.cust_id);
        ...
        if (!IS_NULL(call_type))
        {
                ...
                if (!strcmp(call_type, "call_obd"))
                {
                        const char *dtmf_type = switch_channel_get_variable(channel, "dtmf_value");
                        char dtmf_store[10];
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "ODB's DTMF Value: %s, :: 's' for save DTMF feature, if 2 set then we have IVR ID.", dtmf_type);
                        query = switch_mprintf("UPDATE `pbx_obd_participant` SET `dialed_status` = 'Answered' WHERE id=%s;", participant_id);
                        execute_sql(dsn, query, mutex);
                        call->dp.wel_pmt = switch_mprintf("%s", play_file);
                        call->did.is_outbound_on = true;
                        // handle_default_prompts(call, call->dp.wel_pmt, NULL, NULL, path, custom_path);
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Welcome Path:: %s", call->dp.wel_pmt);

                        if (file_exists(switch_mprintf("%s%s", custom_path, call->dp.wel_pmt)))
                        {
                                call->dp.wel_pmt = switch_mprintf("%s%s", custom_path, call->dp.wel_pmt);
                        }
                        else
                        {
                                call->dp.wel_pmt = NULL;
                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Prompt File Missing. 1 sec pause added");
                                sleep(1);
                        }

                        if (dtmf_type[0] == 's')
                        {
                                switch_play_and_get_digits(session, 1, 1, 1, 5000, "#", call->dp.wel_pmt, NULL, NULL, dtmf_store,
                                                           sizeof(dtmf_store), "[0-9]+", 10000, NULL);

                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, " dtmf_store :%s", (dtmf_store));
                                switch_channel_set_variable(channel, "dtmf_store", dtmf_store);
                        }
                        else if (dtmf_type[0] == '0')
                        {

                                switch_core_session_execute_application(session, "playback", call->dp.wel_pmt);
                        }
                ...
                if (!strcmp(call_type, "Outbound Conference"))
                {
                        char *profile_str = NULL;
                        const char *conf_id = switch_channel_get_variable(channel, "conf_id");
                        call->dp.wel_pmt = switch_mprintf("%s", play_file);
                        query = switch_mprintf("UPDATE `pbx_outbound_conference_participant` SET `dialed_status` = 'Answered' WHERE `pbx_outbound_conference_participant`.`id` = %s;", participant_id);
                        execute_sql(dsn, query, mutex);
                        handle_default_prompts(call, call->dp.wel_pmt, NULL, NULL, path, custom_path);
                        switch_core_session_execute_application(session, "playback", call->dp.wel_pmt);
                        profile_str = switch_mprintf("%s_%s@advc_conf", cust_id, conf_id);
        ...
        if ((!IS_NULL(callee)) && (callee[0] !='*') )
        {
                ...
                else if ((callee[0] >= 48 && callee[0] <= 57) || callee[0] == 43)
                {
                      ...
                      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SIP 5 digit:%s ",callee);
                        
                        if (strlen(callee)  ==5 && call->caller.cust_id != NULL && verify_internal_exten(channel, dsn, mutex, call, callee,path) !=0 )
                              {
                                 switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "cg and conf");
                                   switch_channel_set_variable(channel, "application", "intercom");
                        ...
                        else if (strlen(callee) >= 2 && strlen(callee)  <=5 && !IS_NULL(call->caller.cust_id))
                         {
                        ...
                        else
                        {
                                ...
                                if (verify_did(channel, dsn, mutex,path,custom_path, call))
                                {
                                        
                                      sprintf(id, "%d", call->did.id);
                                      switch_channel_set_variable(channel, "call_back_url", call->did.call_back_url);
                                ...
                                {
                                        switch_channel_set_variable(channel, "application", "outbound");
                                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "PSTN %s : OUTBOUND  : %d  mnt:%d ", callee, call->caller.is_outbound_allowed, call->caller.is_mnt_plan_enabled);
...
originated_header originated_id_manipulation(switch_channel_t *channel, char clr_no[], char clr_name[], call_details_t *call)
{
        char origination_caller_number[] = {'\0'}, caller_id_name[] = {'\0'}, *efc_clr_name, *efc_clr_no;
        switch_core_session_t *session = switch_channel_get_session(channel);
        const char *call_type = switch_channel_get_variable(channel, "call_type");
...
bool  handle_dailout_feature(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path, call_details_t *call,int caller , int feature_id,char *active_feature)
{
        switch_core_session_t *session = switch_channel_get_session(channel);
        char result[128] = {'\0'};
        char *temp_path, *data;
        ...
        if (caller) {
         data = switch_mprintf("call switch_intercom_dialout(%d,%d,'%s')",caller,feature_id,active_feature);
         execute_sql2str(dsn, mutex, data, result, NELEMS(result));
         switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE, " active_feature   %s clr id %d feature_id %d   calling_allowed %d  ",active_feature,caller,feature_id,atoi(result));
         if (atoi(result) != 1)
               {
              switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, " intercoming dialout disable for this feature  please coordinate with admin");
              switch_channel_set_variable(channel, "custom_hangup", "1022");
              temp_path = switch_mprintf("%s%s", path, "dialout_disable.wav");
              switch_ivr_play_file(session, NULL, temp_path, NULL);
              switch_safe_free(temp_path);
              switch_channel_hangup(channel, SWITCH_CAUSE_CALL_REJECTED);
...
bool handle_sip_call(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path, call_details_t *call)
{
        if (clr_status(channel))
        {
                ...
                if (!strcasecmp(call_type, "call_plugin") && IS_NULL(callee))
                {
                        callee = call->caller.num;
                }

                data = switch_mprintf("call switch_callbusy_persist('%s','%s','%s')", call->callee.cust_id, call->callee.num, caller);
                execute_sql2str(dsn, mutex, data, intercom_count, NELEMS(intercom_count));

                if (call->callee.call_persist && intercom_count[0] == '1')
                {

                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "call persist Hangup");
                        temp_path = switch_mprintf("%s%s", path, "busy.wav");
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "callee id:%d  callee:%s  caller:%s   ",call->callee.id,callee, caller);
                switch_safe_free(data);
                ...
                {       
                     ...
                     //    if(!(call->did.is_init) && !(handle_dailout_feature(channel, dsn, mutex,path, custom_path, call,call->caller.id,call->callee.id,"sip")))
                     //  {
                     //   return false;
                     //  }

                        if ((!call->callee.call_waiting && !call->callee.call_persist))
                        {
                                ...
                                if (switch_channel_get_variable(channel, "limit_usage") != NULL)
                                {
                                        ...
                                        else if (atoi(switch_channel_get_variable(channel, "limit_usage")) >= 2)
                                        {
                                                switch_channel_set_variable(channel, "limit_usage", "2");
                                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "clr_uuid:%s", clr_uuid);
                                                temp_path = switch_mprintf("%s%s", path, "busy_on_another_call.wav");
                                                switch_core_session_execute_application(session, "playback", temp_path);
                                                if (call->callee.is_vm_on)
                                                {
                                                        voicemail(session, NULL, NULL, call->callee.num);
                        ...
                        if (call->caller.is_init && !(call->caller.intercom_call))
                        {
                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, " %s Intercom Disabled ", caller);
                                switch_channel_set_variable(channel, "custom_hangup", "1019");
                                temp_path = switch_mprintf("%s%s", path, "not_authorized_to_dial.wav");
                                switch_ivr_play_file(session, NULL, temp_path, NULL);
                                switch_safe_free(temp_path);
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, ":callee recording %s  %s ", callee, call->callee.caller_id_name);
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, ":callee %s name %s ", callee,call->callee.cid_name);
                switch_channel_set_variable(channel, "cle_name", call->callee.cid_name);
                ...
                if (bridge_call(session, call, callee, dsn, mutex))
                {
                        ...
                        if (call->callee.is_call_frwd && !IS_NULL(dialstatus))
                        {
                                int type = -1;
                                if (!strcmp(dialstatus, "BUSY") && call->frwd[1].type)
                                {
                                        type = 1;
                                }
                                else if (!strcmp(dialstatus, "NOANSWER") && call->frwd[2].type)
                                {
                                        type = 2;
                                }
                                else if (!strcmp(dialstatus, "UNALLOCATED_NUMBER") && call->frwd[3].type)
                                {
                                        type = 3;
                                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "UNALLOCATED_NUMBER FORWADING  ");
                                }
...
int handle_cg_stcky_agnt(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path, call_details_t *call)
{
        ...
        and cg.sip like concat('%%',fd.forward,'%%') and fd.src in ('%s','%s') and cg.sticky_agent='1'  ORDER BY fd.hangup_time desc limit 1 ",
                                                 call->cg.id, cg_cust_id, caller, switch_channel_get_variable(channel, "effective_caller_id_number"));
...
void handle_cg(switch_channel_t *channel, call_details_t *call, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path)
{
        if (clr_status(channel))
        {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "clr :%d cg id  %d", call->caller.id, atoi(call->cg.id));
                
                if(!(call->did.is_init) && !handle_dailout_feature(channel, dsn, mutex,path, custom_path, call,call->caller.id,atoi(call->cg.id),"cg"))
                {
                 return ;
                }
                ...
                // welcoming ivr
                sleep(1);
                handle_default_prompts(call, call->cg.wc_pmt_path, NULL, NULL, path, custom_path);
                ...
                if ((dialstatus != NULL && call->cg.un_failover) && (!strcmp(dialstatus, "UNALLOCATED_NUMBER") || !strcmp(dialstatus, "BUSY") || !strcmp(dialstatus, "NOANSWER") || !strcmp(dialstatus, "ALLOTTED_TIMEOUT")))
                {
...
void handle_conf(switch_channel_t *channel, call_details_t *call, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path)
{
        ...
        WHERE pc.customer_id = %s  and pcc.participant_id = %s AND pcc.participants_status like 'active_%%'",
                                    call->conf.cust_id, call->conf.participant_id);
